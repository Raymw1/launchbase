...
module.exports = {
  ...
  async update(req, res, next) {
    ...
    const mailFormat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    if (!email.match(mailFormat)) return res.render("admin/users/edit", { data, error: "Insira um email válido" });
    let user = await User.findOne({ where: { email }, "and not": { id: req.params.id } });
    if (user) return res.render("admin/users/edit", { data, error: "Email já cadastrado!" });
    next();
  }
}